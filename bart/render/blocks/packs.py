"""Domain-pack bart-block renderers — ML / CS / philosophy / math / literature.

ML: ``confusion-matrix`` / ``loss-curve`` / ``neural-net-diagram`` /
``attention-matrix`` / ``embedding-scatter``.  CS: ``code-block`` /
``call-stack`` / ``memory-layout`` / ``binary-tree`` / ``process-timeline``.
Philosophy: ``argument-map`` / ``truth-table`` / ``venn-logic`` /
``dialectic-tree`` / ``quote-pull``.  Math: ``proof-block`` / ``matrix-view`` /
``graph-plot`` / ``integral-area``.  Literature: ``passage-annotated`` /
``character-graph`` / ``theme-weave`` / ``style-spectrum``.  Surfaced in
daily-lesson catalogs when the subject keywords match the domain.

(Was ``lib_blocks_packs.py``.) Internal to ``bart.render.blocks``.
"""
...
from ._shared import _inline_md
